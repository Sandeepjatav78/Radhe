...
const Navbar = () => {
  ...
  return (
    ...
    <div className="sticky sm:px-5 top-0 z-50 w-full bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-100">
      ...
      {/* --- MOBILE SIDEBAR MENU (Solid Background) --- */}
      <div 
      ...
      >
          ...
          <div className="flex flex-col h-full bg-white text-gray-800">
              ...
              <div className="flex flex-col p-4 gap-2 bg-white">
                <NavLink onClick={()=>setVisible(false)} className={({isActive}) => `py-3 pl-6 border rounded-lg text-lg ${isActive ? 'text-emerald-700 bg-emerald-50 border-emerald-200 font-bold' : 'text-gray-700 border-transparent hover:bg-gray-50'}`} to='/'>
                    HOME
                </NavLink>
                <NavLink onClick={()=>setVisible(false)} className={({isActive}) => `py-3 pl-6 border rounded-lg text-lg ${isActive ? 'text-emerald-700 bg-emerald-50 border-emerald-200 font-bold' : 'text-gray-700 border-transparent hover:bg-gray-50'}`} to='/collection'>
                    MEDICINES
                </NavLink>
                <NavLink onClick={()=>setVisible(false)} className={({isActive}) => `py-3 pl-6 border rounded-lg text-lg ${isActive ? 'text-emerald-700 bg-emerald-50 border-emerald-200 font-bold' : 'text-gray-700 border-transparent hover:bg-gray-50'}`} to='/about'>
                    ABOUT
                </NavLink>
                <NavLink onClick={()=>setVisible(false)} className={({isActive}) => `py-3 pl-6 border rounded-lg text-lg ${isActive ? 'text-emerald-700 bg-emerald-50 border-emerald-200 font-bold' : 'text-gray-700 border-transparent hover:bg-gray-50'}`} to='/contact'>
                    CONTACT
                </NavLink>
              </div>
